...
/* --- Defines --- */
#define AZURE_PNP_MODEL_ID "dtmi:iotcentraltestluigi:PlantGuard_1;1"
...
#define SAMPLE_MODEL_PROPERTY_VALUE                    "PlantGuard"
...
#define TELEMETRY_PROP_NAME_MOISTURE                   "moisture1"

static az_span COMMAND_NAME_TOGGLE_PUMP = AZ_SPAN_FROM_STR("pumpoverride1");
#define COMMAND_RESPONSE_CODE_ACCEPTED                 202
#define COMMAND_RESPONSE_CODE_REJECTED                 404

#define WRITABLE_PROPERTY_TELEMETRY_FREQ_SECS          "pumpoverridetestcloud"
...
int azure_pnp_handle_command_request(azure_iot_t* azure_iot, command_request_t command)
{
	...
	if (az_span_is_content_equal(command.command_name, COMMAND_NAME_TOGGLE_PUMP))
	{
		pump_switch = !pump_switch;
		LogInfo("PumpSwitch state: %s", (pump_switch ? "ON" : "OFF"));
...
static float simulated_get_moisture()
{
	return 88.0;
}
...
static int generate_telemetry_payload(uint8_t* payload_buffer, size_t payload_buffer_size, size_t* payload_buffer_length)
{
	...
	moisture = simulated_get_moisture();
	...
	rc = az_json_writer_append_double(&jw, moisture, DOUBLE_DECIMAL_PLACE_DIGITS);
	...
	uint8_t* buffer, size_t buffer_size, size_t* response_length)
{
			...
			AZ_IOT_HUB_CLIENT_PROPERTY_WRITABLE, &component_name)))
	{
		if (az_json_token_is_text_equal(&jr.token, AZ_SPAN_FROM_STR(WRITABLE_PROPERTY_TELEMETRY_FREQ_SECS)))
		{
			...
			azrc = az_json_token_get_boolean(&jr.token, &value);
			EXIT_IF_AZ_FAILED(azrc, RESULT_ERROR, "Failed getting writable properties bool value.");

			// azure_pnp_set_telemetry_frequency((size_t)value);
			LogInfo(String(value).c_str());
